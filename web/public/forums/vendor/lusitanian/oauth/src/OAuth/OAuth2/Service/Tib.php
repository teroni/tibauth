...
class Tib extends AbstractService
{
    ...
    public function getAuthorizationEndpoint()
    {
        ...
        return new Uri('http://10.0.1.11/phpbb/authorize');
    }
    ...
    {
        return new Uri('http://10.0.1.11/phpbb/token');
    }
    public function getUserDataEndpoint()
    {
        return new Uri('http://10.0.1.11/phpbb/resource');
    }
    ...
    public function requestUserData($token)
    {
        $bodyParams = array(
            'access_token'          => $token->access_token,
        );
        $responseBody = $this->httpClient->retrieveResponse($this->getUserDataEndpoint(), $bodyParams, $this->getExtraOAuthHeaders());
        // we can scream what we want that we want bitly to return a json encoded string (format=json), but the
        // WOAH WATCH YOUR LANGUAGE ;) service doesn't seem to like screaming, hence we need to manually parse the result
        return json_decode($responseBody);
    }
